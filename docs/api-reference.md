...
Returns every independent balance. For a unique reaction this is a single result; for underdetermined systems it includes the minimal balance plus one solution per independent free direction.
...
Returns `true` when the equation balances exactly (mass and charge), `false` for any parse or balance failure. Never throws.
...
Reports per-element and charge totals for each side without requiring the equation to balance.
...
Classifies a reaction and reports which elements are oxidised/reduced, using conservative oxidation-state rules (only unambiguous assignments are reported).
...
Structural classification only: `combustion`, `decomposition`, `synthesis`, `single-displacement`, `acid-base` or `metathesis`.

### `oxidationStates(species)`

Average oxidation state per element for a single parsed species. Elements whose states cannot be determined unambiguously (e.g. both Fe and S in `Fe2(SO4)3`) are omitted rather than guessed.
