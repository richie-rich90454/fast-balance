...
describe("acid mine drainage", () => {
    ...
    it("balances Fe2+ + O2 + H2O -> Fe(OH)3", () => {
        try {
            const r = balance("Fe2+ + O2 + H2O -> Fe(OH)3");
            expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
            const all = [
                ...r.reactants.map((x) => x.coefficient),
                ...r.products.map((x) => x.coefficient),
...
describe("silicate formation", () => {
    ...
    it("balances 3MgO + 2SiO2 -> Mg3Si2O5(OH)4", () => {
        try {
            const r = balance("3MgO + 2SiO2 -> Mg3Si2O5(OH)4");
            expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
            const all = [
                ...r.reactants.map((x) => x.coefficient),
                ...r.products.map((x) => x.coefficient),
...
describe("sulfate reduction", () => {
    it("balances SO4^2- + H+ -> H2S + H2O", () => {
        try {
            const r = balance("SO4^2- + H+ -> H2S + H2O");
            expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
            const all = [
                ...r.reactants.map((x) => x.coefficient),
                ...r.products.map((x) => x.coefficient),
    ...
    it("balances SO4^2- + H+ + e- -> H2S + H2O", () => {
        const r = balance("SO4^2- + H+ + e- -> H2S + H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances SO4^2- + organic -> H2S + CO2", () => {
        try {
            const r = balance("SO4^2- + CH2O -> H2S + CO2");
            expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
            const all = [
                ...r.reactants.map((x) => x.coefficient),
                ...r.products.map((x) => x.coefficient),
    ...
    it("balances H2S + O2 -> SO4^2- + H+", () => {
        const r = balance("H2S + O2 -> SO4^2- + H+");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances FeS + O2 -> Fe2O3 + SO4^2-", () => {
        try {
            const r = balance("FeS + O2 -> Fe2O3 + SO4^2-");
            expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
            const all = [
                ...r.reactants.map((x) => x.coefficient),
                ...r.products.map((x) => x.coefficient),
...
describe("iron oxidation", () => {
    ...
    it("balances 4Fe2+ + O2 + H2O -> 4Fe3+ + 4OH-", () => {
        const r = balance("4Fe2+ + O2 + H2O -> 4Fe3+ + 4OH-");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances Fe2+ + H2O + O2 -> Fe(OH)3", () => {
        try {
            const r = balance("Fe2+ + H2O + O2 -> Fe(OH)3");
            expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
            const all = [
                ...r.reactants.map((x) => x.coefficient),
                ...r.products.map((x) => x.coefficient),
    ...
    it("balances 2Fe2O3 -> 4Fe + 3O2", () => {
        const r = balance("2Fe2O3 -> 4Fe + 3O2");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("mineral dissolution", () => {
    ...
    it("balances CaSO4 -> Ca2+ + SO4^2-", () => {
        const r = balance("CaSO4 -> Ca2+ + SO4^2-");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances KAlSi3O8 + H2O + H+ -> Al(OH)3 + H2SiO3 + K+", () => {
        const r = balance("KAlSi3O8 + H2O + H+ -> Al(OH)3 + H2SiO3 + K+");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("soil chemistry", () => {
    ...
    it("balances H+ + clay-O- -> clay-OH", () => {
        try {
            const r = balance("H+ + clay-O- -> clay-OH");
            expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
            const all = [
                ...r.reactants.map((x) => x.coefficient),
                ...r.products.map((x) => x.coefficient),
    ...
    it("balances Ca2+ + 2OH- -> Ca(OH)2", () => {
        const r = balance("Ca2+ + 2OH- -> Ca(OH)2");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances Fe3+ + 3OH- -> Fe(OH)3", () => {
        const r = balance("Fe3+ + 3OH- -> Fe(OH)3");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances NH4+ + OH- -> NH3 + H2O", () => {
        const r = balance("NH4+ + OH- -> NH3 + H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("geochemical cycle", () => {
    ...
    it("balances CaCO3 <-> Ca2+ + CO3^2-", () => {
        const r = balance("CaCO3 <-> Ca2+ + CO3^2-");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("hydrothermal vent", () => {
    it("balances FeS + H2S -> FeS2 + H2", () => {
        const r = balance("FeS + H2S -> FeS2 + H2");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
