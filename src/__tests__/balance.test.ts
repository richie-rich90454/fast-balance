...
describe("mixed parentheses and brackets balance", () => {
    ...
    it("balances [Cu(NH3)4](OH)2 + H2SO4 -> [Cu(NH3)4]SO4 + H2O", () => {
        const result = balance("[Cu(NH3)4](OH)2 + H2SO4 -> [Cu(NH3)4]SO4 + H2O");
        expect(result.reactants.every((r) => r.coefficient > 0)).toBe(true);
        expect(result.products.every((p) => p.coefficient > 0)).toBe(true);
    });
});
...
describe("mass conservation verification", () => {
    it("verifies element balance for H2 + O2 -> H2O", () => {
        const result = balance("H2 + O2 -> H2O");
        const reactantH = result.reactants.reduce(
            (s, r) => s + r.coefficient * (r.formula === "H2" ? 2 : 0),
            0,
        );
        const productH = result.products.reduce(
            (s, p) => s + p.coefficient * (p.formula === "H2O" ? 2 : 0),
            0,
        );
