...
describe("arrow style consistency", () => {
    it("same reaction with '->' and '→' produces same coefficients", () => {
        ...
        expect(r1.reactants.map((s) => s.coefficient)).toEqual(
            r2.reactants.map((s) => s.coefficient),
        );
        expect(r1.products.map((s) => s.coefficient)).toEqual(
            r2.products.map((s) => s.coefficient),
        );
    });
    ...
    it("all arrow styles produce same coefficients for water electrolysis", () => {
        ...
        const coeffs = results.map((r) => [
            ...r.reactants.map((s) => s.coefficient),
            ...r.products.map((s) => s.coefficient),
        ]);
