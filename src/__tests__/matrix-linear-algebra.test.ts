...
describe("buildMatrix dimension tests", () => {
    it("builds 1-element 1-row matrix for a single species", () => {
        const reactants: Species[] = [{ formula: "H", elements: { H: 1 }, charge: 0 }];
    ...
    it("all elements from all species appear as rows in the matrix", () => {
        ...
        expect(matrix.length).toBeGreaterThanOrEqual(Object.keys(elementRows).length);
    });
});
...
describe("solveSystem error handling tests", () => {
    ...
    it("solution is in null space of the matrix", () => {
        const matrix = [[new Fraction(1), new Fraction(-1)]];
        const result = solveSystem(matrix, 2);
        let sum = new Fraction(0);
    ...
    it("scaled solution still satisfies the system", () => {
        ...
        let sum = new Fraction(0);
        for (let j = 0; j < 2; j++) sum = sum.add(matrix[0]![j]!.mul(scaled[j]!));
        expect(sum.isZero()).toBe(true);
    });
...
describe("solveSystem linear algebra property tests", () => {
    ...
    it("result has at least one Fraction that is Fraction.one() in the free column", () => {
        const matrix = [[new Fraction(1), new Fraction(2), new Fraction(3), new Fraction(-6)]];
...
describe("fractionsToIntegers rounding behavior tests", () => {
    it("converts [1/2, 1/2] to [1, 1]", () => {
        expect(fractionsToIntegers([new Fraction(1, 2), new Fraction(1, 2)])).toEqual([1, 1]);
    });

    it("converts [1/3, 2/3] to [1, 2]", () => {
        expect(fractionsToIntegers([new Fraction(1, 3), new Fraction(2, 3)])).toEqual([1, 2]);
    });

    it("converts [2/3, 4/3] to [1, 2] (after GCD)", () => {
        expect(fractionsToIntegers([new Fraction(2, 3), new Fraction(4, 3)])).toEqual([1, 2]);
    });

    it("converts [1/4, 1/2] to [1, 2]", () => {
        expect(fractionsToIntegers([new Fraction(1, 4), new Fraction(1, 2)])).toEqual([1, 2]);
    });

    it("converts [3/2, 1/2] to [3, 1]", () => {
        expect(fractionsToIntegers([new Fraction(3, 2), new Fraction(1, 2)])).toEqual([3, 1]);
    });

    it("all-zero fractions return [0, 0]", () => {
        expect(fractionsToIntegers([new Fraction(0), new Fraction(0)])).toEqual([0, 0]);
    });
...
describe("fractionsToIntegers sign normalization tests", () => {
    it("flips all signs when negatives outnumber positives", () => {
        expect(fractionsToIntegers([new Fraction(-1), new Fraction(-2), new Fraction(1)])).toEqual([
            1, 2, -1,
        ]);
    });
    ...
    it("handles all-positive fractions without sign change", () => {
        expect(fractionsToIntegers([new Fraction(1, 2), new Fraction(3, 2)])).toEqual([1, 3]);
    });
...
describe("fractionsToIntegers GCD reduction tests", () => {
    it("[2, 2] reduces to [1, 1] (GCD=2)", () => {
        expect(fractionsToIntegers([new Fraction(2), new Fraction(2)])).toEqual([1, 1]);
    });

    it("[4, 6] reduces to [2, 3] (GCD=2)", () => {
        expect(fractionsToIntegers([new Fraction(4), new Fraction(6)])).toEqual([2, 3]);
    });

    it("[6, 4] reduces to [3, 2]", () => {
        expect(fractionsToIntegers([new Fraction(6), new Fraction(4)])).toEqual([3, 2]);
    });

    it("[10, 15] reduces to [2, 3]", () => {
        expect(fractionsToIntegers([new Fraction(10), new Fraction(15)])).toEqual([2, 3]);
    });

    it("[100, 50] reduces to [2, 1]", () => {
        expect(fractionsToIntegers([new Fraction(100), new Fraction(50)])).toEqual([2, 1]);
    });

    it("[7, 7] reduces to [1, 1] (already coprime after GCD)", () => {
        expect(fractionsToIntegers([new Fraction(7), new Fraction(7)])).toEqual([1, 1]);
    });
});
