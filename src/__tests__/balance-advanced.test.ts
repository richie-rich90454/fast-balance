...
describe("balance multiple equation consistency", () => {
    it("balance H2+O2->H2O always returns same result", () => {
        ...
        for (const r of results) {
            ...
            expect(r.reactants.map((s) => s.coefficient)).toEqual(
                first.reactants.map((s) => s.coefficient),
            );
            expect(r.products.map((s) => s.coefficient)).toEqual(
                first.products.map((s) => s.coefficient),
            );
        }
    });
