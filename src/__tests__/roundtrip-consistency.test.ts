...
describe("deterministic output", () => {
    it("multiple calls with same input produce same result (H2 + O2 -> H2O)", () => {
        ...
        expect(a.reactants.map((r) => r.coefficient)).toEqual(
            b.reactants.map((r) => r.coefficient),
        );
        expect(b.reactants.map((r) => r.coefficient)).toEqual(
            c.reactants.map((r) => r.coefficient),
        );
    });
    ...
    it("no random behavior across many calls (CH4 + O2 -> CO2 + H2O)", () => {
        ...
        for (let i = 0; i < 25; i++) {
            ...
            expect(result.reactants.map((r) => r.coefficient)).toEqual(
                reference.reactants.map((r) => r.coefficient),
            );
            expect(result.products.map((r) => r.coefficient)).toEqual(
                reference.products.map((r) => r.coefficient),
            );
        }
    });
    ...
    it("consistent across runs (Fe + Cl2 -> FeCl3)", () => {
        ...
        expect(r2.reactants.map((r) => r.coefficient)).toEqual(
            r1.reactants.map((r) => r.coefficient),
        );
        // Product coefficients must be identical arrays
        expect(r2.products.map((r) => r.coefficient)).toEqual(
            r1.products.map((r) => r.coefficient),
        );
        // Equation strings must be identical
        expect(r2.equation).toBe(r1.equation);
        // Formulas must be identical
        expect(r2.reactants.map((r) => r.formula)).toEqual(r1.reactants.map((r) => r.formula));
        expect(r2.products.map((r) => r.formula)).toEqual(r1.products.map((r) => r.formula));
    });
});
...
describe("format roundtrip", () => {
    ...
    it("all formats give same coefficients and array structure (CH4 + O2 -> CO2 + H2O)", () => {
        ...
        expect(text.products.map((r) => r.coefficient)).toEqual(
            html.products.map((r) => r.coefficient),
        );
        expect(text.products.map((r) => r.coefficient)).toEqual(
            latex.products.map((r) => r.coefficient),
        );
        // Formulas must be identical across formats
        expect(text.reactants.map((r) => r.formula)).toEqual(html.reactants.map((r) => r.formula));
        expect(text.reactants.map((r) => r.formula)).toEqual(latex.reactants.map((r) => r.formula));
...
describe("charge conservation consistency", () => {
    ...
    const totalCharge = (
        side: { coefficient: number; formula: string }[],
        charges: number[],
    ): number => {
        let s = 0;
    ...
    it("Fe2+ + Cl- -> FeCl2 conserves charge", () => {
        ...
        const left = totalCharge(
            result.reactants,
            split.reactants.map((s) => s.charge),
        );
        const right = totalCharge(
            result.products,
            split.products.map((s) => s.charge),
        );
...
describe("mass balance consistency", () => {
    ...
    const sideMass = (
        ...
        elementMaps: Record<string, number>[],
    ): number => {
        let total = 0;
    ...
    it("H2 + O2 -> H2O mass is balanced on both sides", () => {
        ...
        const left = sideMass(
            result.reactants,
            split.reactants.map((s) => s.elements),
        );
        const right = sideMass(
            result.products,
            split.products.map((s) => s.elements),
        );
