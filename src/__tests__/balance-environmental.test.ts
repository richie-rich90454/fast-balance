...
describe("ozone depletion", () => {
    it("balances O3 + NO -> NO2 + O2", () => {
        try {
            ...
            expect(e.message).toMatch(
                /Unbalanceable|unbalanced|Cannot balance|Expected element|Impossible|Error/i,
            );
        }
    });
    ...
    it("balances CF2Cl2 -> CF2Cl + Cl", () => {
        const r = balance("CF2Cl2 -> CF2Cl + Cl");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("smog formation", () => {
    ...
    it("balances C2H4 + O3 -> HCHO + CH2O", () => {
        try {
            const r = balance("C2H4 + O3 -> HCHO + CH2O");
            expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
            const all = [
                ...r.reactants.map((x) => x.coefficient),
                ...r.products.map((x) => x.coefficient),
...
describe("fluoridation", () => {
    it("balances H2SiF6 + H2O -> H+ + SiO2 + F-", () => {
        const r = balance("H2SiF6 + H2O -> H+ + SiO2 + F-");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("coagulation and flocculation", () => {
    ...
    it("balances Al3+ + H2O -> Al(OH)3 + H+", () => {
        const r = balance("Al3+ + H2O -> Al(OH)3 + H+");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances Fe3+ + 3H2O -> Fe(OH)3 + 3H+", () => {
        const r = balance("Fe3+ + 3H2O -> Fe(OH)3 + 3H+");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("cation exchange", () => {
    it("balances Ca2+ + 2NaX -> CaX2 + 2Na+", () => {
        const r = balance("Ca2+ + 2NaX -> CaX2 + 2Na+");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances Mg2+ + CaX2 -> MgX2 + Ca2+", () => {
        const r = balance("Mg2+ + CaX2 -> MgX2 + Ca2+");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("soil weathering", () => {
    it("balances KAlSi3O8 + H2O + CO2 -> Al2Si2O5(OH)4 + H4SiO4 + K+ + HCO3-", () => {
        const r = balance("KAlSi3O8 + H2O + CO2 -> Al2Si2O5(OH)4 + H4SiO4 + K+ + HCO3-");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances CaMg(CO3)2 + H2O + CO2 -> Ca2+ + Mg2+ + HCO3-", () => {
        const r = balance("CaMg(CO3)2 + H2O + CO2 -> Ca2+ + Mg2+ + HCO3-");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances Fe2O3 + 6H+ -> 2Fe3+ + 3H2O", () => {
        const r = balance("Fe2O3 + 6H+ -> 2Fe3+ + 3H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances Al2O3 + 6H+ -> 2Al3+ + 3H2O", () => {
        const r = balance("Al2O3 + 6H+ -> 2Al3+ + 3H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("nitrification", () => {
    it("balances NH3 + O2 -> HNO2 + H2O", () => {
        const r = balance("NH3 + O2 -> HNO2 + H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances NH4+ + O2 -> NO2- + H+ + H2O", () => {
        const r = balance("NH4+ + O2 -> NO2- + H+ + H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("denitrification", () => {
    ...
    it("balances NO3- + CH3OH -> N2 + CO2 + H2O", () => {
        try {
            const r = balance("NO3- + CH3OH -> N2 + CO2 + H2O");
            expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
            const all = [
                ...r.reactants.map((x) => x.coefficient),
                ...r.products.map((x) => x.coefficient),
...
describe("ammonification", () => {
    ...
    it("balances C5H7NO2 + O2 -> CO2 + H2O + NH3", () => {
        const r = balance("C5H7NO2 + O2 -> CO2 + H2O + NH3");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("sulfur oxidation", () => {
    ...
    it("balances H2S + O3 -> SO2 + H2O", () => {
        const r = balance("H2S + O3 -> SO2 + H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("sulfur reduction", () => {
    it("balances SO4^2- + CH2O + H+ -> H2S + CO2 + H2O", () => {
        const r = balance("SO4^2- + CH2O + H+ -> H2S + CO2 + H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances SO4^2- + H+ + e- -> S + H2O", () => {
        const r = balance("SO4^2- + H+ + e- -> S + H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("methane oxidation", () => {
    ...
    it("balances CH4 + OH -> CH3 + H2O", () => {
        const r = balance("CH4 + OH -> CH3 + H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("CFC decomposition", () => {
    it("balances CFCl3 -> CFCl2 + Cl", () => {
        const r = balance("CFCl3 -> CFCl2 + Cl");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("heavy metal removal - precipitation", () => {
    ...
    it("balances Pb2+ + 2OH- -> Pb(OH)2", () => {
        const r = balance("Pb2+ + 2OH- -> Pb(OH)2");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances Cr3+ + 3OH- -> Cr(OH)3", () => {
        const r = balance("Cr3+ + 3OH- -> Cr(OH)3");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("heavy metal removal - redox", () => {
    ...
    it("balances CrO4^2- + H+ -> Cr3+ + H2O", () => {
        try {
            const r = balance("CrO4^2- + H+ -> Cr3+ + H2O");
            expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
            const all = [
                ...r.reactants.map((x) => x.coefficient),
                ...r.products.map((x) => x.coefficient),
    ...
    it("balances MnO4- + Fe2+ + H+ -> Mn2+ + Fe3+ + H2O", () => {
        const r = balance("MnO4- + Fe2+ + H+ -> Mn2+ + Fe3+ + H2O");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("organic pollutant degradation", () => {
    ...
    it("balances CCl4 + Fe0 -> C + Fe2+ + Cl-", () => {
        const r = balance("CCl4 + Fe -> C + Fe2+ + Cl-");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
...
describe("phosphate removal", () => {
    it("balances PO4^3- + Al3+ -> AlPO4", () => {
        const r = balance("PO4^3- + Al3+ -> AlPO4");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances PO4^3- + Fe3+ -> FePO4", () => {
        const r = balance("PO4^3- + Fe3+ -> FePO4");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
    ...
    it("balances PO4^3- + Ca2+ -> Ca3(PO4)2", () => {
        const r = balance("PO4^3- + Ca2+ -> Ca3(PO4)2");
        expect(r.reactants.every((s) => s.coefficient > 0)).toBe(true);
        const all = [
            ...r.reactants.map((x) => x.coefficient),
            ...r.products.map((x) => x.coefficient),
